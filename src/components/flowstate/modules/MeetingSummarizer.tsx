...
import { exportAsPdf, emailDraft } from "@/lib/exports";
import { trackUse, trackConfidence } from "@/lib/analytics";
...
export function MeetingSummarizer() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const actions = data.action_items.map((a, i) => ({
        id: `a${i}`,
        text: a.task ?? "",
      ...
      const decisions = data.decisions.map((d, i) => ({
        id: `d${i}`,
        text: d.text ?? "",
        confidence: typeof d.confidence === "number" ? Math.max(0, Math.min(1, d.confidence)) : 0.5,
      }));
      setResult({ summary: data.summary, actions, decisions, followUp: data.follow_up ?? [] });
      trackUse("meeting");
      trackConfidence([...actions.map((a) => a.confidence), ...decisions.map((d) => d.confidence)]);
  ...
  const buildPlainSummary = () => {
    ...
    return [
      ...
      "",
      "ACTION ITEMS",
      ...result.actions.map((a) => `• ${a.text} — ${a.owner} (due ${a.due})`),
      "",
      "KEY DECISIONS",
  ...
  const handlePdf = () => {
    ...
    const html = `
      ...
      <div class="meta">Generated by FlowState · ${new Date().toLocaleString()}</div>
      <h2>Executive summary</h2>
      <p>${escapeHtml(result.summary).replace(/\n/g, "<br/>")}</p>
      <h2>Action items</h2>
      <ul>${result.actions.map((a) => `<li><strong>${escapeHtml(a.text)}</strong> — ${escapeHtml(a.owner)} · due ${escapeHtml(a.due)}</li>`).join("")}</ul>
      <h2>Key decisions</h2>
      <ul>${result.decisions.map((d) => `<li>${escapeHtml(d.text)}</li>`).join("")}</ul>`;
    exportAsPdf("Meeting summary", html);
    fire("success");
  };

  const handleEmail = () => {
    emailDraft("Meeting summary", buildPlainSummary());
    fire("success");
  };
      ...
      <SectionShell
        ...
        action={result && (
          <div className="flex items-center gap-1.5">
            ...
            >
              <Download className="h-3.5 w-3.5" /> Export PDF<Flash kind={flash} />
            </button>
            <button
              onClick={handleEmail}
              className="inline-flex items-center gap-1.5 h-8 px-2.5 rounded-md text-xs border border-border hover:bg-muted"
            ...
            {tab === "summary" && (
              <div className="space-y-4">
                {!typed ? (
                  <TypedText
                    text={result.summary}
                    className="text-sm leading-relaxed p-3 rounded-lg border border-input bg-surface min-h-[120px]"
