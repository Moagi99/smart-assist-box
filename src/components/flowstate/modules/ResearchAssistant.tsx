...
import { Search, Sparkles, RotateCcw, ShieldCheck, AlertTriangle, Download, Copy } from "lucide-react";
...
export function ResearchAssistant() {
  ...
  const buildMarkdown = () => {
    ...
    const lines = [
      ...
      "",
      "## Key Claims",
      ...result.key_claims.map((c) => `- **[${c.strength.toUpperCase()}]** ${c.text}${c.counter_argument ? `\n  - _Counter:_ ${c.counter_argument}` : ""}`),
      "",
      "## Evidence Quality",
      result.evidence_quality,
      "",
      "## Bias Flags",
      ...result.bias_flags.map((b) => `- ${b}`),
      "",
      "## Recommendations",
      ...result.recommendations.map((r, i) => `${i + 1}. ${r}`),
      "",
      `_Generated by FlowState · ${new Date().toLocaleString()}_`,
    ];
    return lines.join("\n");
  };
  ...
  const exportMd = () => {
    if (!result) return;
    downloadBlob(buildMarkdown(), "research-analysis.md", "text/markdown;charset=utf-8");
    fire("success");
  ...
  const copyCitations = async () => {
    if (!result) return;
    const cites = result.key_claims.map((c, i) => `[${i + 1}] (${c.strength}) ${c.text}`).join("\n");
    try { await copyText(cites); fire("success"); toast.success("Citations copied"); }
      ...
      <SectionShell
        ...
        action={result && (
          <div className="flex items-center gap-1.5">
            <button onClick={copyCitations} className="inline-flex items-center gap-1.5 h-8 px-2.5 rounded-md text-xs border border-border hover:bg-muted">
              <Copy className="h-3.5 w-3.5" /> Citations<Flash kind={flash} />
            </button>
            <button onClick={exportMd} className="inline-flex items-center gap-1.5 h-8 px-2.5 rounded-md text-xs border border-border hover:bg-muted">
        ...
        {loading ? (
          ...
          <div className="space-y-5">
            <div>
              ...
              {!typed ? (
                <TypedText text={result.tldr} className="text-sm leading-relaxed" onDone={() => setTyped(true)} />
              ) : (
